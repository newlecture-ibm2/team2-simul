...
### 🌿 Branch 3: `feat/SCRUM-172` — 통합 검색 및 태그 자동완성
**PR:** `[TAG/feat] 통합 검색 및 태그 자동완성 풀스택 구현` | **예상:** 약 9시간

#### 백엔드 (4h)
| 이슈 | 상태 | 설명 | 예상 |
|---|---|---|---|
| **SCRUM-172** | ✅ 완료 | 태그 자동완성 검색 API (`GET /tags/search?q=`, `usage_count` 내림차순) | 2h |
| **SCRUM-173** | ✅ 완료 | 통합 검색 API (`GET /search?q=&type=tag/caption/all`, 페이지네이션) | 2h |

**구현 상세:**
- `SearchController.java`: 헥사고날 Inbound Adapter로 통합 검색 오케스트레이션 API 신규 구현.
- `PostService.searchPosts()`: `#` 기호 유무에 따라 `tagQuery`와 `captionQuery`를 분리하여 검색하는 정교한 로직 적용.
- `PostJpaRepository.java`: 캡션 LIKE 검색, 태그 정확 매칭 검색, 통합(태그+캡션) 검색용 JPQL 쿼리 3종 추가.
- `TagJpaRepository`: `findByNameStartingWithIgnoreCaseOrderByUsageCountDesc`로 인기 태그 우선순위 자동완성 구현.
...
**구현 상세:**
- `SearchBar`: 디바운싱(300ms) 자동완성 드롭다운, `#` 입력 시 태그 모드 전환, `initialQuery` prop으로 URL 상태 동기화.
- `SearchFilter`: 전체/태그/본문 검색 타입 글래스모피즘 토글.
- `SearchResultGrid`: 2열 메이슨리 레이아웃 + Intersection Observer 무한 스크롤.
- **태그 클릭 연동 (Instagram 패턴)**: 피드 PostCard, 검색 PostCard, 게시물 상세 페이지의 모든 `#태그`를 클릭 가능한 Link로 변경 → `/search?q=%23태그&type=tag`로 즉시 이동.
- **뒤로가기 상태 유지**: `useSearchParams` + `router.replace`로 검색어/필터를 URL 쿼리 파라미터와 실시간 동기화. 상세 페이지 방문 후 뒤로가기 시 이전 검색 결과 완벽 복원.
- `Header.tsx`: 돋보기 아이콘을 `/search` 페이지 Link로 연결.
