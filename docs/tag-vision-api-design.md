...
### 🌿 Branch 2: `feat/reports` — 신고/블라인드 및 비로그인 제어
**PR:** `[POST/feat] 게시물 신고/블라인드 시스템 및 비로그인 처리` | **예상:** 약 7시간+

#### 백엔드 (4h)
| 이슈 | 설명 | 예상 |
|---|---|---|
| **SCRUM-153** | reports 테이블 Entity 및 Repository 구현 | 1h |
| **SCRUM-154** | 게시물 신고 API (`POST /posts/{postId}/report`, `UNIQUE(post_id, reporter_id)`) | 1h |
| **SCRUM-155** | 신고 N회 누적 자동 블라인드 트리거 구현 (10회 → `is_blinded=true`) | 2h |

#### 프론트엔드 (3h)
| 이슈 | 설명 | 예상 |
|---|---|---|
| **SCRUM-163** | 게시물 신고 UI 구현 (사유 입력 모달) | 1h |
| **SCRUM-165** | 비로그인 사용자 피드 열람 및 로그인 유도 바텀시트 | 2h |

#### 핵심 산출물
- `Report.java` (Entity, `UNIQUE(post_id, reporter_id)`) + `ReportJpaRepository` + `ReportPersistenceAdapter`
- `PostController` → `POST /posts/{postId}/report`
- 자동 블라인드 비즈니스 로직: 신고 저장 후 `report_count` 증가 → 10 이상이면 `is_blinded = true` 업데이트
- `app/(main)/post/[id]/_components/ReportModal/` (신고 사유 선택/입력 UI)
- 비로그인 로그인 유도 바텀시트 공용 컴포넌트
