...
단순한 형태 분석의 한계를 극복하기 위해 **기능 다중 호출**과 **화이트리스트 방어막**을 사용합니다. 이 핵심 로직은 `GoogleVisionTagAdapter.java` 클래스(`backend/src/main/java/com/simul/tag/adapter/out/vision/GoogleVisionTagAdapter.java`)에 구현되어 있습니다.

1. **다중 호출 (Dual Detection):**
   - `LABEL_DETECTION` (최대 30개): "Denim", "Fashion" 등 소재 및 전체적인 분위기를 폭넓게 추출.
   - `OBJECT_LOCALIZATION` (최대 15개): "Top", "Pants", "Shoe" 등 구체적인 의류 객체(사물)를 정확히 위치 기반으로 감지하여 추출.
...
| ~~SCRUM-172~~ | ~~TAG-005~~ | ~~태그 자동완성 검색 API 구현~~ ✅ 구현 완료 |
| ~~SCRUM-173~~ | ~~TAG-006~~ | ~~통합 검색 API 구현~~ ✅ 구현 완료 |
| SCRUM-174 | TAG-007 | 게시물 작성 태그 자동완성 UI 구현 |
| ~~SCRUM-175~~ | ~~TAG-008~~ | ~~통합 검색 페이지 구현~~ ✅ 구현 완료 |
| ~~SCRUM-176~~ | ~~TAG-009~~ | ~~피드 카드 태그 칩 표시 UI 구현~~ ✅ 구현 완료 |
| ~~SCRUM-177~~ | ~~TAG-010~~ | ~~게시물 상세 태그 목록 UI 구현~~ ✅ 완료 (#75) |
| ~~SCRUM-178~~ | ~~TAG-011~~ | ~~태그/검색 API 클라이언트 연동~~ ✅ 구현 완료 |
...
### 🌿 Branch 3: `feat/SCRUM-172` — 이미지-태그 생명주기 연동 및 통합 검색
**PR:** `[POST/fix] 이미지-태그 생명주기 연동 및 정렬/삭제 정합성 통합 해결`

#### 백엔드 
| 이슈 | 상태 | 설명 |
|---|---|---|
| **SCRUM-172** | ✅ 완료 | 태그 자동완성 검색 API 및 이미지별 태그 매핑(`imageTagsMap`) 도메인 개편 |
| **SCRUM-173** | ✅ 완료 | 통합 검색 API (`GET /search?q=&type=tag/caption/all`, 페이지네이션) |

**구현 상세:**
- **태그-이미지 생명주기 매핑**: `AttachTagsToPostUseCase`에 `attachTagsWithSource`를 도입하여, 이미지(`imageUrl`)와 추출된 태그 간의 관계를 추적. 이미지가 삭제되거나 순서(`sortOrder`)가 변경되어도 태그 정합성이 유지되도록 구조 개편.
- **태그 중복 정화 로직**: 게시물 수정 시 `HashSet`을 활용하여 중복된 태그로 인한 DB 유니크 제약 조건(`Unique Constraint`) 충돌 완벽 방지.
- **통합 검색 고도화**: `SearchController.java` 오케스트레이션 및 `#` 기호 유무에 따른 `tagQuery` / `captionQuery` 분리 검색 로직 적용 (`PostJpaRepository` 3종 쿼리 추가).
- **자동완성**: `TagJpaRepository`의 `findByNameStartingWithIgnoreCaseOrderByUsageCountDesc`로 인기 태그 우선순위 자동완성.

#### 프론트엔드 (2h)
| 이슈 | 상태 | 설명 | 예상 |
|---|---|---|---|
| **SCRUM-175** | ✅ 완료 | 통합 검색 페이지 구현 (`app/(main)/search/page.tsx`) | 2h |

**구현 상세:**
- **드래그 앤 드롭 태그 연동**: `@dnd-kit`을 활용한 이미지 재배치 시, 연관된 태그 정보가 서버로 정확히 전달(`imageOrderJson`)되도록 상태 관리 최적화.
- **상세 페이지 UX**: 이미지별 태그 분리 렌더링 지원 및 좋아요 클릭 시 `bouncyHeart` 탄성 애니메이션 추가.
- `SearchBar` & `SearchFilter`: 디바운싱 자동완성 드롭다운, 전체/태그/본문 검색 토글 및 URL 상태 동기화.
- **태그 클릭 연동**: 피드/검색 PostCard 및 상세 페이지의 `#태그` 클릭 시 `/search?q=%23태그&type=tag`로 즉시 이동.

#### 테스트/연동 (3h)
| 이슈 | 상태 | 설명 | 예상 |
|---|---|---|---|
| **SCRUM-178** | ✅ 완료 | 태그/검색 API 클라이언트 연동 (`tagAPI.ts`, `searchAPI.ts`, `lib/api/index.ts` re-export) | 1h |
| **SCRUM-179** | 🔲 미착수 | 태그/검색 단위 테스트 및 UI 테스트 | 2h |
...
### 🌿 Branch 5: `feat/SCRUM-157` — 커뮤니티 피드 편의성 고도화 (정렬 및 팔로잉 연동)
**PR:** `[POST/feat] 커뮤니티 정렬 토글 구현 및 팔로잉 피드 연동`

#### 1. 정렬 토글 (Sort Toggle) 및 인기글 정책
- **이슈:** 누적 좋아요(`likeCount`) 기반의 단순 인기순 정렬 시 발생하는 "고인물 게시물(장기 상위권 독점)" 현상.
- **해결 방안 (기간 한정 필터링):** `PostJpaRepository`에 `CreatedAtAfter` 조건을 추가하여 **주간 인기글(최근 7일)**로 기한을 제한함.
- **구현 로직:** `PostService`에서 정렬 조건이 `popular`일 경우, `LocalDateTime.now().minusDays(7)`을 `since` 파라미터로 넘겨 트렌디한 피드 생태계를 유지하도록 설계.

#### 2. 팔로잉 피드 (Following Feed) 도메인 통신
- **이슈:** `Post` 도메인 내에서 피드를 구성해야 하나, 대상 유저가 팔로우하는 사용자 목록은 `User` 도메인이 소유함.
- **해결 방안 (헥사고날 규칙 준수):** `PostService`가 `User` 도메인의 내부 DB 포트(`FollowPersistencePort`)에 직접 접근하는 것을 엄격히 차단하고, 반드시 **Input Port(`LoadFollowUseCase`)**를 통과하도록 구조화.
- **구현 로직:** `LoadFollowUseCase` 인터페이스에 `getFollowingIds()` 메서드를 개방하고, `PostService`에서 이를 주입받아 팔로잉 피드를 구성함.

#### 프론트엔드
- `CommunityPage` 상단에 `[🔄 최신순 | 인기순]`을 변경할 수 있는 글래스모피즘 형태의 정렬 토글 UI(`SortToggle`) 추가 및 Flex/Sticky 레이아웃 반응형 보정.

---

## 6. 오픈 그래프(Open Graph) 메타 태그 및 공유 기능 (SCRUM-146)

게시물 상세 페이지를 카카오톡, 슬랙, 인스타그램 등 소셜 미디어와 메신저로 공유할 때, 텍스트 링크만 노출되는 문제를 해결하고 바이럴 효율을 높이기 위해 **OG 메타 태그(Open Graph Meta Tags)**를 동적으로 생성합니다.

### 6.1. 공유 기능 흐름 (Web Share API)
1. **네이티브 경험 우선**: 사용자가 헤더의 공유 버튼 클릭 시 브라우저 내장 `navigator.share()` API를 호출하여 OS 네이티브 공유 시트(카카오톡, 메시지 등)를 즉시 오픈합니다.
2. **폴백(Fallback)**: Web Share API를 지원하지 않는 데스크톱 브라우저의 경우, 클립보드에 URL을 복사하고 `toast.success`로 알림을 제공합니다.

### 6.2. Next.js `generateMetadata` 연동 설계
Next.js App Router의 `generateMetadata`를 활용하여 서버 사이드(SSR)에서 각 게시물 ID(`postId`)에 맞는 메타 태그를 `<head>`에 동적으로 주입합니다. 크롤러 봇이 접속했을 때 아래의 태그를 읽어 미리보기 카드를 렌더링합니다.

- **`og:title`**: `[SIMUL] {nickname}님의 스타일`
- **`og:description`**: 게시물 캡션 첫 50자 요약 (없을 경우 기본 문구 제공)
- **`og:image`**: 게시물의 첫 번째 이미지(`images[0].imageUrl`) 절대 경로
- **`og:type`**: `article` (게시물 형태)

*주의*: 메신저 크롤러는 자바스크립트를 실행하지 않는 경우가 많으므로, 이 작업은 반드시 서버 컴포넌트(`page.tsx` 또는 `layout.tsx`) 단계에서 처리되어야 합니다.
...
## 10. 주간 QA 및 테스트 계획 (FEED & TAG 도메인 테스트 주간)

이번 주차는 그동안 구현한 핵심 도메인 로직과 UI 컴포넌트의 신뢰성을 확보하기 위해 **단위 테스트(Unit Test) 및 통합 테스트(Integration Test)** 구현에 집중합니다. 첨부된 지라(Jira) 백로그를 기반으로 아래와 같은 테스트 시나리오를 작성하고 검증할 예정입니다.

### 🧪 주요 테스트 타겟 및 시나리오

| 지라 이슈 / 컴포넌트 | 테스트 범위 | 핵심 검증 시나리오 |
|---|---|---|
| **SCRUM-142** | `[FEED]` Vision API 태그 추천 UI | - 이미지 업로드 후 Vision API 모킹(Mocking) 응답이 추천 태그 칩(Chip) UI로 잘 렌더링되는가?<br>- 사용자가 추천 태그를 클릭하여 활성화/비활성화할 수 있는가? |
| **SCRUM-157** | `[FEED]` 커뮤니티 정렬 토글 & 팔로잉 | - **[백엔드]** `sort=popular` 파라미터가 주어졌을 때 7일 이전 게시물은 배제되고 좋아요 순으로 정렬되는가?<br>- **[백엔드]** `tab=following` 파라미터 시 타 도메인(`User`)을 정상적으로 호출하여 올바른 ID 리스트를 가져오는가?<br>- **[프론트]** 정렬 토글 클릭 시 즉각적으로 상태가 변경되고 올바른 API가 호출되는가? |
| **SCRUM-167** | `[FEED]` 피드 페이지 UI 테스트 | - 피드 그리드(FeedGrid)의 컴포넌트 렌더링 검증 및 스켈레톤 로딩(Skeleton Loading) 테스트.<br>- 비어있는 상태(Empty State) 디자인 노출 여부 점검. |
| **SwipePage** | `[HOME]` 메인 스와이프 페이지 테스트 | - **[제스처]** 우측(좋아요), 상/하(다음/이전) 스와이프 이벤트 시 올바른 애니메이션과 API가 트리거되는가?<br>- **[에러 대응]** API 500 에러 또는 빈 배열 반환 시 `DUMMY_POSTS`가 화면에 정상 렌더링되어 서비스 중단을 막는가?<br>- **[비로그인]** 비로그인 상태에서 우측 스와이프(좋아요) 시 토스트 에러 메시지가 표출되는가? |
| **SCRUM-174** | `[TAG]` 게시물 작성 태그 자동완성 UI | - 해시태그(`#`) 입력 시 서버에 자동완성 쿼리가 디바운싱(Debouncing)되어 올바르게 전달되는가?<br>- 태그 개수 초과(10개) 시 경고 메시지 토스트(Toast)가 정상 동작하는가? |

> **테스트 목표:** 이번 주 테스트 작성 후 테스트 커버리지 최소 70% 이상 달성 및 Edge Case(토큰 만료, 잘못된 응답, 데이터 없음 등) 대비.
