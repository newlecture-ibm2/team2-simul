...
> **작성/업데이트:** 2026-05-07
> **상태:** 4차 구현 완료 (게시글 수정/삭제 CRUD 완전 구현, PC 스와이프 버그 및 모바일 UI 개선, Tag 업데이트 연동)
...
| SCRUM-144 | FEED-004 | feat/145 | 게시물 상세 조회 API 구현 |
| SCRUM-145 | FEED-005 | feat/145 | 게시물 수정 및 삭제(Soft Delete) API 및 UI 구현 |
| SCRUM-159 | FEED-019 | feat/145 | 게시물 상세 페이지 구현 (PC Drag-to-scroll 적용) |
| SCRUM-164 | FEED-024 | feat/145 | 피드 CRUD API 클라이언트 연동 완료 |
...
> **기준일:** 2026-05-03 ~ | `fix/SCRUM-143` PR 머지 후 순차 착수
...
### 🏃 Sprint 1 — 게시물 상세 API + 좋아요 시스템 (백엔드 집중)
**브랜치:** `feat/SCRUM-144` | **PR:** `[POST/feat] 게시물 상세 조회 API 및 좋아요 토글 구현`

| 이슈 | 설명 | 예상 |
|---|---|---|
| **SCRUM-144** | 게시물 상세 조회 API (`GET /api/posts/{postId}`) | 1.5h |
| **SCRUM-147** | likes 테이블 Entity 및 Repository | 1h |
| **SCRUM-148** | 좋아요 토글 API (`POST /api/posts/{postId}/likes`) | 1.5h |

> **병합 근거:** 상세 API에 `isLiked` 필드가 필요 → likes Entity 선행 필수 → 함께 진행이 효율적

**핵심 산출물:**
- `Like.java` (Entity) + `LikeJpaRepository` + `LikePersistenceAdapter`
- `GetPostDetailUseCase` + `ToggleLikeUseCase` (Input Port)
- `PostController` → `GET /api/posts/{postId}`, `POST /api/posts/{postId}/likes`

### 🏃 Sprint 2 — 게시물 상세 페이지 + 좋아요 + 정렬 (프론트 집중)
**브랜치:** `feat/SCRUM-159` | **PR:** `[POST/feat] 게시물 상세 페이지 API 연동 및 정렬 토글 UI`

| 이슈 | 설명 | 예상 |
|---|---|---|
| **SCRUM-159** | 게시물 상세 페이지 API 연동 (더미→실데이터) | 1.5h |
| **SCRUM-161** | 좋아요 버튼 UI + 낙관적 업데이트 + 롤백 | 1h |
| **SCRUM-157** | 최신순/인기순 정렬 토글 (백엔드 이미 지원) | 1h |
| **SCRUM-177** (TAG-010) | 게시물 상세 태그 목록 UI | 0.5h |

> **병합 근거:** 모두 프론트엔드, Sprint 1의 백엔드 API에 의존 → 같은 PR로 묶으면 자연스러움

**핵심 산출물:**
- `app/(main)/post/[id]/page.tsx` API 연동 + `_components/` 분리
- `app/(main)/_components/SortToggle/` 정렬 토글 컴포넌트
- `PostCard.tsx` → `toggleLike` API 호출 + 실패 롤백

### 🏃 Sprint 3 — 댓글 시스템 풀스택
**브랜치:** `feat/SCRUM-149` | **PR:** `[POST/feat] 댓글 시스템 풀스택 구현 (Entity→API→UI)`

| 이슈 | 설명 | 예상 |
|---|---|---|
| **SCRUM-149** | comments 테이블 Entity + Repository | 0.5h |
| **SCRUM-150** | 댓글 목록 조회 API | 1h |
| **SCRUM-151** | 댓글 작성 API (`parent_comment_id` 대댓글 포함) | 0.5h |
| **SCRUM-152** | 댓글 삭제 API (소프트 딜리트) | 0.5h |
| **SCRUM-162** | 댓글 목록/작성/대댓글 UI | 1.5h |

> **병합 근거:** 댓글은 Entity→CRUD API→UI가 하나의 기능 단위 → 풀스택 1 PR

**핵심 산출물:**
- `Comment.java` + `CommentJpaRepository` + `CommentPersistenceAdapter`
- `PostController` → `GET/POST /api/posts/{postId}/comments`, `DELETE /api/comments/{commentId}`
- `app/(main)/post/[id]/_components/CommentSection/`

### 🏃 Sprint 4 — 게시물 삭제 + 신고 시스템 + 비로그인 처리
**브랜치:** `feat/SCRUM-153` | **PR:** `[POST/feat] 게시물 삭제, 신고/블라인드 시스템 및 비로그인 처리`

| 이슈 | 설명 | 예상 |
|---|---|---|
| **SCRUM-145** | 게시물 삭제 API (소프트 딜리트) | 0.5h |
| **SCRUM-153** | reports 테이블 Entity + Repository | 0.5h |
| **SCRUM-154** | 게시물 신고 API (`UNIQUE(post_id, reporter_id)`) | 1h |
| **SCRUM-155** | 신고 5회 누적 → 자동 블라인드 트리거 | 0.5h |
| **SCRUM-163** | 게시물 신고 UI (사유 입력 모달) | 1h |
| **SCRUM-165** | 비로그인 사용자 피드 열람 + 로그인 유도 바텀시트 | 0.5h |

> **병합 근거:** 삭제/신고/블라인드는 모두 "게시물 보호 정책" → 같은 맥락으로 1 PR

**핵심 산출물:**
- `Report.java` + `ReportJpaRepository` + `ReportPersistenceAdapter`
- `PostController` → `DELETE /api/posts/{postId}`, `POST /api/posts/{postId}/report`
- `app/(main)/post/[id]/_components/ReportModal/`
- 비로그인 로그인 유도 바텀시트 컴포넌트
